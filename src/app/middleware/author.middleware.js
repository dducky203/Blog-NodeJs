...
export const checkAuthorIdInBody = async (req, res, next) => {
    const _id = req.body.author_id;
    ...
    if (_id) {
        if (isValidObjectId(_id)) {
            ...
            if (!author) {
                return responseError(res, 404, `Tác giả với id: ${_id} không tồn tại hoặc đã bị xóa`);
            }
        }
    }
    next();
